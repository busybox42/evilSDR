...
class SDRAudioProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    // If we've run completely dry, re-gate to rebuild buffer
    if (this.buffered === 0) {
      this.playing = false;
      channel.fill(0);
      return true;
    ...
    // Catch up if buffer is too large (more than 1s lag)
    if (this.buffered > 48000) {
       this.readPos = (this.writePos - 9600 + this.bufferSize) % this.bufferSize;
       this.buffered = 9600;
       console.log('[Audio] Buffer catch-up triggered');
    }
